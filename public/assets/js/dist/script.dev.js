...
var error = false;
var divResult = document.querySelector("div.form-results"); // Control name field

var signupName = document.querySelector("input[name='signupName']");
signupName.addEventListener("focusout", function (e) {
  if (signupName.value.length < 3) {
    signupName.style.borderColor = "red"; // create and print label

    var label = createLabel("Name must be at least 3 characters long", "signup-name"); // Check if this label already exists
    ...
    signupName.style.borderColor = "green"; // If next sibling is label, remove it
...
var signupEmail = document.querySelector("input[name='signupEmail']");
signupEmail.addEventListener("focusout", function (e) {
  if (isEmailAddressValid(signupEmail.value)) {
    ...
    var label = createLabel("The email address is not valid.", "signup-email");
...
var signupTelephone = document.querySelector("input[name='signupTelno']");
signupTelephone.addEventListener("focusout", function (e) {
  ...
  if (signupTelephone.value.match(pattern)) {
    ...
    var label = createLabel("The telephone number is not valid.", "signup-telno");

    if (!checkNextTag("LABEL", signupTelephone)) {
      signupTelephone.parentNode.insertBefore(label, signupTelephone.nextSibling);
    }
...
var signupPassword = document.querySelector("input[name='signupPasskey']");
signupPassword.addEventListener("focusout", function (e) {
  if (isPasswordValid(signupPassword.value)) {
    ...
    var label = createLabel("The password is not valid.", "signup-passkey");
...
var signupConfirmPassword = document.querySelector("input[name='signupPasskeyConfirm']");
signupConfirmPassword.addEventListener("focusout", function (e) {
  if (signupConfirmPassword.value === signupPassword.value) {
    ...
    if (checkNextTag("LABEL", signupConfirmPassword)) {
      signupConfirmPassword.parentNode.removeChild(signupConfirmPassword.nextSibling);
    }
  } else {
    signupConfirmPassword.style.borderColor = "red";
    var label = createLabel("The passwords do not match.", "signup-passkey-confirm");

    if (!checkNextTag("LABEL", signupConfirmPassword)) {
      signupConfirmPassword.parentNode.insertBefore(label, signupConfirmPassword.nextSibling);
    }
...
var signupTerms = document.querySelector("input[name='signupTerm']");
signupTerms.addEventListener("focusout", function (e) {
  if (signupTerms.checked) {
    ...
    var label = createLabel("You must agree to the terms and conditions.", "signup-term");
...
var signupSubmit = document.querySelector("input[name='signupSubmit']");
signupSubmit.addEventListener("click", function (e) {
  // check if all fields are valid before submitting form data to server
  if (signupName.value.length >= 3 && signupEmail.value.length >= 3 && signupTelephone.value.length >= 3 && signupPassword.value.length >= 3 && signupConfirmPassword.value === signupPassword.value && signupTerms.checked) {
    ...
    e.preventDefault();
    var label = createLabel("Please fill out all fields correctly.", "signup-submit");
...
var loginEmail = document.querySelector("input[name='loginEmail']");
...
loginEmail.addEventListener("focusout", function (e) {
  ...
  if (loginEmail.value.length >= 3) {
    ...
    var label = createLabel("The email is not valid.", "login-email");
...
var loginPasskey = document.querySelector("input[name='loginPasskey']");
loginPasskey.addEventListener("focusout", function (e) {
  if (loginPasskey.value.length >= 3) {
    if (isPasswordValid(loginPasskey.value)) {
      ...
      var label = createLabel("The password is not valid.", "login-passkey");
...
var loginSubmit = document.querySelector("input[id='loginSubmit']");
loginSubmit.addEventListener("click", function (e) {
  if (!loginError) {
    ...
    e.preventDefault();
    var label = createLabel("Please fill out all fields correctly.", "login-submit");
